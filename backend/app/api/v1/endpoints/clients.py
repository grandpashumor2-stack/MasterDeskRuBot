...
async def list_clients(company_id: UUID, skip: int = Query(0, ge=0), limit: int = Query(50, ge=1, le=200), session: AsyncSession = Depends(get_db), current_user=Depends(get_current_owner)):
