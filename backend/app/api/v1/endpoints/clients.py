...
Примеры маппинга:
- "стучит подвеска", "трясёт на кочках" → service_keywords: ["диагностика подвески", "подвеска"]
- "тянет вправо", "уводит в сторону" → service_keywords: ["развал-схождение", "сход-развал"]
- "кондиционер не холодит", "плохо охлаждает" → service_keywords: ["кондиционер", "диагностика кондиционера"]  
- "замена масла", "поменять масло" → service_keywords: ["замена масла", "масло", "ТО"]
- "тормоза скрипят" → service_keywords: ["тормоза", "колодки", "тормозные колодки"]
- "не заводится" → service_keywords: ["диагностика", "двигатель"]

Верни ТОЛЬКО JSON без пояснений."""


CHAT_SYSTEM_PROMPT = """Ты — вежливый и профессиональный AI-администратор автосервиса "{company_name}".
...
3. НИКОГДА не придумывай цены — используй только данные из контекста
4. Если цена не найдена — предложи оставить заявку
5. Предлагай записаться на ремонт
6. Уточняй марку и модель автомобиля для точной цены
...
Если клиент спрашивает о цене услуги:
- Найди услугу в списке выше
- Если есть цена по марке — уточни марку авто
- Если цены нет — скажи "уточним стоимость индивидуально"

Если клиент хочет записаться:
- Уточни услугу, марку авто
- Попроси номер телефона
- Скажи что мастер подберёт время
...
async def detect_intent(message: str) -> dict:
    ...
    try:
        response = await client.messages.create(
            ...
            max_tokens=300,
            messages=[
                {"role": "user", "content": f"{INTENT_DETECTION_PROMPT}\n\nСообщение: {message}"}
            ]
        )
...
def build_services_info(services: list[Service]) -> str:
    """Build services info string for AI context."""
    lines = []
    for svc in services:
        if not svc.is_active:
            continue
        line = f"• {svc.name} (длительность: {svc.duration_minutes} мин)"
        if svc.prices:
            price = svc.prices[0]
            if price.price_type == PriceType.FIXED:
                ...
                makes_str = ", ".join([f"{k}: {int(v):,} ₽".replace(",", " ") for k, v in price.prices_by_make.items()])
                line += f" — цены по маркам: {makes_str}"
